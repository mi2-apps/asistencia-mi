...
export const devManualPages = pgTable(
  "dev_manual_pages",
  {
    ...
    createdByApiKeyId: varchar("created_by_api_key_id").references(() => apiKeys.id, {
      onDelete: "set null",
    }),
    lastEditedBy: varchar("last_edited_by").references(() => users.id, { onDelete: "set null" }),
    lastEditedByApiKeyId: varchar("last_edited_by_api_key_id").references(() => apiKeys.id, {
      onDelete: "set null",
    }),
  ...
  (table) => ({
    sectionOrderIdx: index("idx_dev_manual_pages_section_order").on(
      table.section,
      table.displayOrder,
    ),
...
export const devManualRevisions = pgTable(
  "dev_manual_revisions",
  {
    id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
    pageId: varchar("page_id")
      .notNull()
      .references(() => devManualPages.id, { onDelete: "cascade" }),
    version: integer("version").notNull(),
    title: text("title").notNull(),
    contentMd: text("content_md").notNull(),
    changeSummary: text("change_summary"),
    editedBy: varchar("edited_by").references(() => users.id, { onDelete: "set null" }),
    editedByApiKeyId: varchar("edited_by_api_key_id").references(() => apiKeys.id, {
      onDelete: "set null",
    }),
    createdAt: timestamp("created_at", { mode: "string" }).notNull().default(sql`now()`),
  },
  (table) => ({
    pageVersionUnique: unique("uq_dev_manual_revisions_page_version").on(
      table.pageId,
      table.version,
    ),
...
export const devManualDictionary = pgTable(
  "dev_manual_dictionary",
  {
    id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
...
export const insertDevManualPageSchema = createInsertSchema(devManualPages).omit({
  id: true,
  version: true,
  editLockVersion: true,
  createdAt: true,
  updatedAt: true,
  createdBy: true,
  createdByApiKeyId: true,
  lastEditedBy: true,
  lastEditedByApiKeyId: true,
});
