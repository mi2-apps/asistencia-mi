...
import {
  createDevManualPage,
  deleteDevManualPage,
  getDevManualPageById,
  getDevManualPageBySlug,
  getDevManualRevision,
  getDictOverview,
  getDictTable,
  listDevManualPages,
  listDevManualRevisions,
  listDictTables,
  revertDevManualPage,
  searchDevManual,
  searchDictColumns,
...
const requireManualAccess = (req: AuthRequest, res: Response, next: any) => {
  ...
  if (role !== "admin" && role !== "supervisor") {
    return res
      .status(403)
      .json({ error: "Developer manual access requires admin or supervisor role" });
  }
  next();
};
...
router.get("/overview", async (_req, res) => {
  try {
    res.json(await getDictOverview());
  } catch (e) {
    console.error("devManual overview", e);
    res.status(500).json({ error: "Failed" });
  }
});
router.get("/dictionary/tables", async (_req, res) => {
  try {
    res.json(await listDictTables());
  } catch (e) {
    console.error("devManual dict tables", e);
    res.status(500).json({ error: "Failed" });
  }
});
router.get("/dictionary/columns", async (req, res) => {
  try {
    res.json(
      await searchDictColumns(
        String(req.query.q || ""),
        req.query.table ? String(req.query.table) : undefined,
      ),
    );
  } catch (e) {
    console.error("devManual dict columns", e);
    res.status(500).json({ error: "Failed" });
  }
});
router.get("/dictionary/tables/:table", async (req, res) => {
  try {
    ...
    console.error("devManual dict table", e);
    res.status(500).json({ error: "Failed" });
  }
});
...
router.get("/pages", async (_req, res) => {
  try {
    ...
    console.error("devManual list", e);
...
router.get("/search", async (req, res) => {
  try {
    ...
    console.error("devManual search", e);
...
router.post("/pages", async (req: AuthRequest, res) => {
  try {
    ...
    if (!slug || !title || !contentMd)
      return res.status(400).json({ error: "slug, title and contentMd are required" });
    ...
    console.error("devManual create", e);
...
router.put("/pages/:id", async (req: AuthRequest, res) => {
  try {
    const { title, summary, section, contentMd, displayOrder, isPublished, changeSummary } =
      req.body || {};
    ...
    if (!result.ok && result.reason === "conflict") {
      return res.status(409).json({
        error: "Edit conflict — the page changed since you loaded it. Reload and reapply.",
        currentLockVersion: result.currentLockVersion,
      });
    }
    res.json((result as any).page);
  } catch (e) {
    console.error("devManual update", e);
...
router.get("/pages/:id/history", async (req, res) => {
  try {
    ...
    console.error("devManual history", e);
...
router.get("/pages/:id/history/:version", async (req, res) => {
  try {
    ...
    console.error("devManual revision", e);
...
router.post("/pages/:id/revert/:version", async (req: AuthRequest, res) => {
  try {
    const result = await revertDevManualPage(
      req.params.id,
    ...
    if (!result.ok && result.reason === "not_found")
      return res.status(404).json({ error: "Page or revision not found" });
    if (!result.ok && result.reason === "conflict") {
      return res
        .status(409)
        .json({ error: "Edit conflict on revert.", currentLockVersion: result.currentLockVersion });
    }
    res.json((result as any).page);
  } catch (e) {
    console.error("devManual revert", e);
    res.status(500).json({ error: "Failed to revert page" });
  }
});
...
router.delete("/pages/:id", requireAdmin, async (req, res) => {
  try {
    ...
    console.error("devManual delete", e);
