...
function buildServer(): McpServer {
  const server = new McpServer({
    name: "__APP_NAME__-developer-manual" /* [EDIT] your app slug */,
    version: "1.0.0",
  });
  ...
  server.registerTool(
    ...
    {
      description:
        "Discovery doc for the MI AI Manifest data model: counts, domains, and how to query. Call this first.",
      inputSchema: {},
    },
  ...
  server.registerTool(
    "dev_manual_list_tables",
    {
      description: "List every documented database table with its domain and purpose.",
      inputSchema: {},
    },
  ...
  server.registerTool(
    "dev_manual_get_table",
    {
      description:
        "Get a table's full definition: purpose, foreign keys, sample queries, and every column's meaning, type, nullability, and allowed values. Use this instead of guessing what a column means.",
      inputSchema: { table: z.string().describe("Exact SQL table name, e.g. 'rail_shipments'") },
    },
    async ({ table }) => {
      const t = await getDictTable(table);
      return asText(
        t ?? {
          error: `Table '${table}' is not documented. Call dev_manual_list_tables for valid names.`,
        },
      );
    },
  );
  ...
  server.registerTool(
    "dev_manual_find_field",
    {
      description:
        "Find what a field/column means across all tables (answers 'what is X?', 'what is qty the quantity of?', 'what does seg mean?'). Optionally restrict to one table.",
      inputSchema: {
        query: z.string().describe("Field name or concept to look up"),
        table: z.string().optional().describe("Optional table to restrict to"),
      },
    },
  ...
  server.registerTool(
    ...
    {
      description:
        "Full-text search of the narrative manual pages (domain overviews, relationships, data flows, glossary).",
  ...
  server.registerTool(
    "dev_manual_read_page",
    {
      description:
        "Read a full narrative manual page (Markdown) by slug, e.g. 'rail', 'pallets', 'glossary', 'relationships', 'data-flows'.",
      inputSchema: { slug: z.string() },
    },
    async ({ slug }) => {
      const p = await getDevManualPageBySlug(slug);
      return asText(
        p
          ? { slug: p.slug, title: p.title, contentMd: p.contentMd }
