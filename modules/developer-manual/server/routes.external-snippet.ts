...
router.post("/developer-manual/mcp", requirePermission("developer_manual", "read"), (req, res) =>
  devManualMcpPost(req, res),
);
router.get("/developer-manual/mcp", requirePermission("developer_manual", "read"), (req, res) =>
  devManualMcpSession(req, res),
);
router.delete("/developer-manual/mcp", requirePermission("developer_manual", "read"), (req, res) =>
  devManualMcpSession(req, res),
);
...
router.get(
  ...
  requirePermission("developer_manual", "read"),
  async (_req, res) => {
    res.json({ data: await getDictOverview() });
  },
);
router.get(
  "/developer-manual/dictionary/tables",
  requirePermission("developer_manual", "read"),
  async (_req, res) => {
...
router.get(
  "/developer-manual/dictionary/columns",
  requirePermission("developer_manual", "read"),
  async (req, res) => {
    res.json({
      data: await searchDictColumns(
        String(req.query.q || ""),
...
router.get(
  "/developer-manual/dictionary/tables/:table",
  requirePermission("developer_manual", "read"),
  async (req, res) => {
...
router.post(
  "/developer-manual/pages",
  requirePermission("developer_manual", "write"),
  async (req, res) => {
    ...
    if (await getDevManualPageBySlug(slug, true))
...
router.put(
  ...
  async (req, res) => {
    ...
    if (!result.ok && result.reason === "conflict")
      return res
        .status(409)
        .json({ error: "edit_conflict", currentLockVersion: result.currentLockVersion });
    res.json({ data: (result as any).page });
  },
);
router.get(
  "/developer-manual/pages/:id/history",
  requirePermission("developer_manual", "read"),
  async (req, res) => {
...
router.get(
  "/developer-manual/pages/:id/history/:version",
  requirePermission("developer_manual", "read"),
  async (req, res) => {
...
router.post(
  "/developer-manual/pages/:id/revert/:version",
  requirePermission("developer_manual", "write"),
  async (req, res) => {
