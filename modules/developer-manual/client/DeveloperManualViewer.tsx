...
import { ArrowLeft, BookOpen, Loader2, Pencil, Save, Search, ShieldAlert, X } from "lucide-react";
...
export default function DeveloperManualViewer() {
  ...
  const { data: pages = [], isLoading: pagesLoading } = useQuery<PageListItem[]>({
    queryKey: ["/api/developer-manual/pages"],
    enabled: !!hasAccess,
  });
  const { data: page, isLoading: pageLoading } = useQuery<PageFull>({
    queryKey: ["/api/developer-manual/pages", slug],
    enabled: !!hasAccess,
  });
  ...
  const { data: results = [] } = useQuery<SearchHit[]>({
    ...
    queryFn: async () =>
      (
        await apiRequest("GET", `/api/developer-manual/search?q=${encodeURIComponent(term)}`)
      ).json(),
    ...
    onError: (e: any) => {
      ...
      toast({
        ...
        description: conflict
          ? "This page changed since you opened it. Reloading the latest version — reapply your edit."
          : e?.message || "Could not save.",
        variant: "destructive",
      });
      if (conflict)
        queryClient.invalidateQueries({ queryKey: ["/api/developer-manual/pages", slug] });
    },
  });
      ...
      <div className="flex h-screen items-center justify-center p-6">
        <div className="max-w-md text-center space-y-3">
          ...
          <p className="text-muted-foreground">
            This area is restricted to admin and supervisor accounts. AI agents access it via the
            Developer Manual API with a read-scoped key.
          </p>
          <Link href="/">
              ...
              <div key={section} className="mb-3">
                ...
                {items.map((p) => (
                  <Link key={p.id} href={`/developer-manual/${p.slug}`}>
                    <div
                      className={`px-2 py-1.5 rounded text-sm cursor-pointer ${p.slug === slug ? "bg-accent font-medium" : "hover:bg-accent/50"}`}
                    >
                      {p.title}
                    </div>
                  </Link>
            ...
            <>
              <div className="flex items-start justify-between mb-4 gap-4">
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() => {
                    setDraft({ title: page.title, contentMd: page.contentMd, changeSummary: "" });
                    setEditing(true);
                  }}
                >
