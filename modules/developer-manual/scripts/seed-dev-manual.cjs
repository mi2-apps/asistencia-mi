...
const PG = {
  ...
  user: process.env.PGUSER || "master",
  password: process.env.PGPASSWORD || "master",
  database: DB,
};
...
const API_PAGE = {
  slug: "developer-manual-api",
  title: "Developer Manual API (editing)",
  section: "Guide",
  order: 4,
...
async function main() {
  ...
  try {
    ...
    const entries = [];
    const files = fs.existsSync(SEED_DIR)
    ...
    for (const f of files.sort()) {
      if (f.startsWith("_")) {
        ...
        entries.push({
          file: f,
          slug: g ? g.slug : f.replace(/^_|\.md$/g, ""),
          section: "Guide",
          order: g ? g.order : 5,
          title: g && g.title,
        });
    ...
    if (fs.existsSync(DICT_DIR)) {
      ...
      const isc = await client.query(
        "SELECT table_name, column_name, is_nullable FROM information_schema.columns WHERE table_schema='public'",
      );
      for (const r of isc.rows)
        nullMap.set(r.table_name + "." + r.column_name, r.is_nullable === "YES");
      let tCount = 0,
        cCount = 0;
      for (const [tableName, { domainKey, t }] of byTable) {
        ...
        await client.query(
          ...
          [
            ...
            domainKey,
            t.purpose || null,
            JSON.stringify(t.foreignKeys || []),
            JSON.stringify(t.sampleQueries || []),
            cols.length,
          ],
        );
        tCount++;
        for (let i = 0; i < cols.length; i++) {
          ...
          let nullable =
            typeof c.nullable === "boolean"
              ? c.nullable
              : String(c.nullable).toLowerCase() === "true" ||
                  String(c.nullable).toLowerCase() === "yes"
                ? true
                : c.nullable == null
                  ? null
                  : false;
