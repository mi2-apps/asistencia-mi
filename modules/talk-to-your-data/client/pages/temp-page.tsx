...
import { CalendarClock, Loader2, MessageSquare, RefreshCw, Wand2, X } from "lucide-react";
...
export function TempPage() {
  ...
  async function doExtend() {
    setExtending(true);
    try {
      const res = await fetch(`/api/v1/pages/${encodeURIComponent(slug)}/extend`, {
        method: "POST",
        credentials: "include",
      });
      if (res.ok) {
        const j = await res.json();
        setExtendedTo(j.expiresAt);
      }
    } finally {
      setExtending(false);
    }
    ...
    <div className="anim-up space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-3 text-2xs text-muted-foreground">
          {p.chat_id && (
            <Link
              href={`/chat?thread=${encodeURIComponent(p.chat_id)}`}
          ...
          <span className="inline-flex items-center gap-1">
            ...
            {t("temp.expires", { d: String(expires).slice(0, 10) })}
          </span>
          <button
            onClick={doExtend}
            disabled={extending}
      ...
      <div className="glass-card p-3 space-y-2">
        ...
        >
          ...
          <input
            ...
            disabled={refining}
            placeholder={t("temp.refinePlaceholder")}
        ...
        {applied && (
          <div className="flex items-center gap-2 text-2xs">
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-primary/15 text-primary">
              ...
              <button onClick={resetRefine} className="hover:text-foreground">
                <X className="w-3 h-3" />
              </button>
            </span>
