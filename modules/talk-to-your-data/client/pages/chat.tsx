...
import { Database, ExternalLink, Filter, Loader2, Plus, Send, Sparkles } from "lucide-react";
...
const TOOL_LABEL: Record<string, string> = {
  ...
  describe_schema: "Reading schema",
  render_block: "Rendering",
  create_page: "Building page",
  set_filters: "Filtering dashboard",
};
...
export function Chat() {
  ...
  const appendText = (text: string) =>
    patchLast((parts) => {
      ...
      if (lp && lp.kind === "text") {
        parts[parts.length - 1] = { kind: "text", text: lp.text + text };
        return parts;
      }
      return [...parts, { kind: "text", text }];
    });
  const finishTool = (name: string, detail?: string) =>
    patchLast((parts) => {
      for (let i = parts.length - 1; i >= 0; i--) {
        const p = parts[i];
        if (p.kind === "tool" && p.name === name && p.status === "start") {
          parts[i] = { ...p, status: "done", detail };
          return parts;
        }
      }
  ...
  async function recoverAnswer(userMessage: string) {
    ...
    if (!id) {
      pushPart({ kind: "note", text: t("chat.connLost") });
      return;
    }
    ...
    for (let i = 0; i < 18; i++) {
      ...
      try {
        const d = await fetch(`/api/v1/chat/${encodeURIComponent(id)}`, {
          credentials: "include",
        }).then((r) => (r.ok ? r.json() : null));
        ...
        for (let j = msgs.length - 1; j >= 0; j--) {
          if (msgs[j].role === "user" && msgs[j].text === userMessage) {
            for (let k = j + 1; k < msgs.length; k++) {
              if (msgs[k].role === "assistant" && msgs[k].text?.trim()) {
                appendText("\n\n" + msgs[k].text);
                return;
              }
            }
  ...
  async function send(text: string) {
    ...
    try {
      ...
      if (!resp.ok || !resp.body) {
        ...
        pushPart({ kind: "note", text: j?.error?.message ?? t("chat.unavailable") });
        return;
      }
      ...
      while (true) {
        ...
        while ((idx = buf.indexOf("\n\n")) >= 0) {
          ...
          let e: any;
          try {
            e = JSON.parse(frame.slice(frame.indexOf("{")));
          } catch {
            continue;
          }
          if (e.type === "chat") {
            ...
            localStorage.setItem(LS_KEY, e.chatId);
          } else if (e.type === "token") appendText(e.text);
          else if (e.type === "tool") {
            if (e.status === "start")
              pushPart({ kind: "tool", name: e.name, status: "start", detail: e.detail });
            else finishTool(e.name, e.detail);
          } else if (e.type === "block") pushPart({ kind: "block", block: e.block });
          else if (e.type === "page")
            pushPart({ kind: "page", url: e.url, title: e.title, expiresAt: e.expiresAt });
  ...
  return (
    ...
    <div className="anim-up flex flex-col min-h-[70vh] md:h-[calc(100vh-3.5rem)]">
          ...
          <>
            {threads.length > 0 && (
              <select
                value={activeId}
                disabled={busy}
                onChange={(e) => loadThread(e.target.value)}
                className="ctrl px-2 py-1.5 text-xs max-w-[200px]"
                title={t("chat.history")}
              >
                {!activeId && <option value="">{t("chat.history")}</option>}
                {threads.map((th) => (
                  <option key={th.id} value={th.id}>
                    {(th.title || "Chat").slice(0, 40)}
                  </option>
                ))}
              </select>
            )}
      ...
      <div ref={scroller} className="flex-1 overflow-auto space-y-4 py-3 pr-1">
        ...
        {messages.map((m, mi) => (
          <div key={mi} className={m.role === "user" ? "flex justify-end" : ""}>
            {m.role === "user" ? (
              <div className="max-w-[85%] rounded-lg bg-primary/15 px-3.5 py-2 text-sm">
              ...
              <div className="max-w-full space-y-2.5">
                {m.parts.map((p, pi) => {
                  if (p.kind === "text")
                    return p.text ? (
                      <div key={pi} className="text-sm leading-relaxed whitespace-pre-wrap">
                        {p.text}
                      </div>
                    ) : null;
                  if (p.kind === "tool")
                    return (
                      <div
                        key={pi}
                        className="inline-flex items-center gap-1.5 text-2xs text-muted-foreground bg-surface-2 rounded px-2 py-1 mr-2"
                      >
                        {p.status === "start" ? (
                  ...
                  if (p.kind === "page")
                    return (
                      <a
                        key={pi}
                        href={p.url}
                        className="glass-card flex items-center gap-3 p-3 hover:bg-surface-2 transition-colors"
                      >
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="text-2xs text-muted-foreground">
                            {t("chat.pageLink")}
                            {p.expiresAt
                              ? ` · ${t("chat.expires", { d: String(p.expiresAt).slice(0, 10) })}`
                              : ""}
                          </div>
                        </div>
                      </a>
      ...
      >
        ...
        <button
          type="submit"
          disabled={enabled === false || busy || !input.trim()}
