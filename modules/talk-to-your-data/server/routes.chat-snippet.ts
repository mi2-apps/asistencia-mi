...
api.get("/chat/logs", (req, res) =>
  ok(async () => {
    ...
    const rows = await query<any>(
     ...
     FROM agent_chats ORDER BY updated_at DESC LIMIT ?`,
      [limit],
    );
    return rows.map((r) => {
      ...
      for (let i = msgs.length - 1; i >= 0; i--) {
        ...
        if (m?.role === "assistant" && typeof m.content === "string" && m.content) {
          lastReply = m.content.slice(0, 800);
          break;
        }
      }
      ...
      for (const m of msgs) {
        if (m?.role === "tool" && typeof m.content === "string" && m.content.includes('"error"')) {
          try {
            ...
            if (j?.error) lastError = String(j.error);
          } catch {
            /* */
          }
        }
...
api.post("/chat", async (req: any, res) => {
  ...
  if (!message)
    return res.status(400).json({ error: { code: "bad_request", message: "message required" } });
  if (!openrouterReady)
    return res.status(503).json({
      error: {
        code: "not_configured",
        message: "Chat is not configured — set OPENROUTER_API_KEY.",
      },
    });
  ...
  let history: any[] = [];
  if (req.body?.chatId) {
    const row = await queryOne<{ messages: any[] }>(
      "SELECT messages FROM agent_chats WHERE id = ?",
      [chatId],
    ).catch(() => undefined);
  ...
  try {
    ...
    await query(
      ...
      [
        ...
        owner ?? null,
        title,
        JSON.stringify(result.messages).slice(0, 4_000_000),
        JSON.stringify(result.sqlLog).slice(0, 500_000),
        result.cost,
      ],
...
api.get("/chats", (req: any, res) =>
  ok(async () => {
    const owner = ownerOf(req);
    if (!owner) return [];
    return query(
      `SELECT id, COALESCE(NULLIF(title,''), 'Chat') AS title, updated_at
     FROM agent_chats WHERE owner = ? ORDER BY updated_at DESC LIMIT 30`,
      [owner],
    );
  })(req, res),
);
...
api.get("/chat/:id", (req: any, res) =>
  ok(async () => {
    const owner = ownerOf(req);
    const row = await queryOne<{ owner: string | null; messages: any[] }>(
      "SELECT owner, messages FROM agent_chats WHERE id = ?",
...
api.get("/pages/:slug", async (req, res) => {
  try {
    ...
    if (!meta)
      return res
        .status(404)
        .json({ error: { code: "not_found", message: "page not found or expired" } });
    ...
    res.json({
      ...
      chat_id: (meta as any).chat_id ?? null,
      created_at: meta.created_at,
      expires_at: meta.expires_at,
      extended_count: meta.extended_count,
...
api.post("/pages/:slug/refine", async (req: any, res) => {
  ...
  if (!instruction)
    return res
      .status(400)
      .json({ error: { code: "bad_request", message: "instruction required" } });
  if (!openrouterReady)
    return res
      .status(503)
      .json({ error: { code: "not_configured", message: "chat not configured" } });
  try {
    const meta = await pages.getPageMeta(String(req.params.slug));
    if (!meta)
      return res
    ...
    await runAgent({
      message: msg,
      owner: ownerOf(req),
      chatId: meta.chat_id ?? undefined,
      lang: String(req.body?.lang ?? ""),
      emit: (e) => {
        if (e.type === "block") collected.push((e as any).block);
      },
    });
    ...
    res
      .status(500)
      .json({ error: { code: "refine_failed", message: String(err?.message ?? err) } });
  }
});

api.post("/pages/:slug/extend", (req, res) =>
  ok(async () => {
...
api.delete("/pages/:slug", (req: any, res) =>
  ok(() => pages.deletePage(String(req.params.slug), ownerOf(req)))(req, res),
);
