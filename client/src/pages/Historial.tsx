...
import { DEPARTAMENTOS_LIST, TIPOS_INASISTENCIA } from "@shared/constants";
import { useMutation, useQuery, useQueryClient } from "@tanstack/react-query";
import { ChevronLeft, ChevronRight, Edit2, Search, X } from "lucide-react";
...
export default function Historial() {
  ...
  const [editandoDia, setEditandoDia] = useState<{ colaborador_id: number; fecha: string } | null>(
    null,
  );
  ...
  const { data, isLoading } = useQuery<{ registros: RegistroSemana[]; bajas_count: number }>({
    ...
    queryFn: () =>
      fetch(`/api/v1/asistencia/semana?inicio=${inicio}&fin=${fin}`, {
        credentials: "include",
      }).then((r) => r.json()),
  });
  ...
  const { data: teData, isLoading: teLoading } = useQuery<{
    success: boolean;
    registros: RegistroTE[];
  }>({
    queryKey: ["historial-te", inicio, finTE],
    queryFn: () =>
      fetch(`/api/v1/tiempo-extra?inicio=${inicio}&fin=${finTE}`, { credentials: "include" }).then(
        (r) => r.json(),
      ),
  });
  ...
  const byColaboradorTE = useMemo(() => {
    ...
    for (const r of teData?.registros ?? []) {
      if (!map.has(r.colaborador_id)) map.set(r.colaborador_id, { info: r, dias: new Map() });
  ...
  const kpis = useMemo(() => {
    ...
    return {
      activos: byColaborador.length,
      bajas: data?.bajas_count ?? 0,
      presentes,
      inasistencias,
      sinRegistrar,
      promedio,
    ...
    <div className="p-4 md:p-6">
      ...
      <div className="md:hidden mb-4 space-y-3">
        ...
        <div className="flex gap-2">
          <select
            ...
            className="flex-1 py-1.5 px-2 text-xs border border-input rounded-md bg-background focus:outline-none"
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-2 md:gap-3 mb-4 md:mb-5">
        ...
        <div className="rounded-xl border border-border bg-card p-2.5 md:p-4">
          <p className="text-[9px] md:text-[10px] font-semibold uppercase tracking-wider text-muted-foreground mb-1.5">
            {t("historial:kpiEmployees")}
          </p>
          <div className="flex gap-2 md:gap-4">
            <div>
              <p className="text-[9px] text-muted-foreground uppercase">
                {t("historial:kpiActive")}
              </p>
        ...
        <div className="rounded-xl border border-border bg-card p-2.5 md:p-4">
          ...
          <p className="text-[10px] md:text-xs text-muted-foreground mt-0.5 leading-tight">
            {t("historial:kpiPresentOf", { present: kpis.presentes, total: kpis.totalRegistros })}
          </p>
        </div>
        {/* Inasistencias */}
        <div className="rounded-xl border border-border bg-card p-2.5 md:p-4">
          <p className="text-[9px] md:text-[10px] font-semibold uppercase tracking-wider text-muted-foreground mb-1.5">
            {t("historial:kpiAbsences")}
          </p>
          <p className="text-lg md:text-2xl font-bold text-red-600">{kpis.inasistencias}</p>
          <p className="text-[10px] md:text-xs text-muted-foreground mt-0.5 leading-tight">
            {t("historial:kpiAbsencesDesc")}
          </p>
        </div>
        {/* Desglose */}
        <div className="rounded-xl border border-border bg-card p-2.5 md:p-4">
          <p className="text-[9px] md:text-[10px] font-semibold uppercase tracking-wider text-muted-foreground mb-1.5">
            {t("historial:kpiBreakdown")}
          </p>
          <div className="flex gap-2 md:gap-3">
            <div>
              <p className="text-[9px] text-green-600 uppercase font-medium">
                {t("historial:kpiPresent")}
              </p>
              <p className="text-base md:text-lg font-bold text-green-600">{kpis.presentes}</p>
            </div>
            <div className="w-px bg-border" />
            <div>
              <p className="text-[9px] text-red-500 uppercase font-medium">
                {t("historial:kpiAbsent")}
              </p>
              <p className="text-base md:text-lg font-bold text-red-500">{kpis.inasistencias}</p>
            </div>
            <div className="w-px bg-border" />
            <div>
              <p className="text-[9px] text-amber-500 uppercase font-medium leading-tight">
                SIN
                <br />
                REG.
              </p>
          ...
          <table className="w-full text-sm table-fixed">
            <thead>
              <tr className="border-b border-border bg-muted/40">
                <th className="text-left px-2 md:px-4 py-3 font-medium text-muted-foreground w-[160px] md:w-72">
                  {t("historial:employee")}
                </th>
                {dias.map((d) => (
                  <th
                    key={d.toISOString()}
                    className="text-center py-3 font-medium text-muted-foreground"
                  >
                    ...
                    <td className="px-2 md:px-4 py-2">
                      <div className="flex items-center gap-1.5">
                        <Avatar
                          ...
                          fotoPerfil={info.foto_perfil}
                          size="sm"
                          className="flex-shrink-0"
                        />
                        <div className="min-w-0">
                          <p className="font-medium leading-tight text-[11px] md:text-sm">
                            {info.fullname}
                          </p>
                      ...
                      return (
                        <td key={iso} className="text-center px-0.5 md:px-2 py-2">
                          {!reg ? (
                            ...
                            <span className="inline-block px-1 md:px-2 py-0.5 rounded-full bg-green-100 text-green-700 text-[10px] md:text-xs font-medium">
                              ✓
                            </span>
                          ) : (
                            <span
                              className={cn(
                                "inline-block px-1 md:px-2 py-0.5 rounded-full text-[10px] md:text-xs font-medium",
                                TIPO_COLORS[reg.tipo_inasistencia ?? "FI"] ??
              ...
              {teLoading ? (
                ...
                colaboradoresFiltradosTE.map(({ info, dias: dm }) => (
                  <tr
                    ...
                    className="border-b border-border/50 hover:bg-muted/30 cursor-pointer"
                    onClick={() => {
                      const asist = byColaborador.find(
                        (c) => c.info.colaborador_id === info.colaborador_id,
                      );
                  ...
                  >
                    ...
                    {diasTE.map((d) => {
                      ...
                      const totalHoras = regs.reduce(
                        (acc, r) => acc + parseFloat(r.horas_totales ?? "0"),
                        0,
                      );
                      const horasStr = Number.isInteger(totalHoras)
                        ? `${totalHoras}h`
                      ...
                      return (
                        <td key={iso} className="text-center px-0.5 md:px-2 py-2">
                          <span className="inline-flex flex-col items-center gap-0.5 px-1 md:px-2 py-1 rounded-lg bg-amber-50 border border-amber-200">
                            <span className="text-[10px] md:text-xs font-semibold text-amber-700">
                              {horasStr}
                            </span>
                            <span className="text-[9px] md:text-[10px] text-amber-600 whitespace-nowrap hidden md:block">
                              {horario}
                            </span>
                          </span>
                        </td>
          ...
          >
            ...
            <div className="p-5">
              <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-3">
                Asistencia &middot;{" "}
                {lunes.toLocaleDateString("es-MX", { day: "numeric", month: "short" })} &ndash;{" "}
                {sabado.toLocaleDateString("es-MX", { day: "numeric", month: "short" })}
              </p>
              <div>
                {dias.map((d) => {
                  ...
                  const isEditing =
                    editandoDia?.fecha === iso &&
                    editandoDia?.colaborador_id === colabModal.info.colaborador_id;
                  return (
                          ...
                          <>
                            <div className="flex-1 flex flex-col gap-0.5">
                              <div className="flex items-center gap-2 flex-wrap">
                                {!reg ? (
                                  ...
                                  <span className="inline-block px-2.5 py-0.5 rounded-full bg-green-100 text-green-700 text-xs font-medium">
                                    Presente ✓
                                  </span>
                                ) : (
                                  <>
                                    ...
                                    >
                                      {TIPOS_INASISTENCIA.find(
                                        (ti) => ti.code === reg.tipo_inasistencia,
                                      )?.label ??
                                        reg.tipo_inasistencia ??
                                        "Inasistencia"}
                                    </span>
                            ...
                            <button
                              onClick={() => {
                                ...
                                setEditEstado(
                                  (reg?.estado as "Presente" | "Inasistencia") ?? "Presente",
                                );
                            ...
                            if (count === 1)
                              return (
                                <div className="rounded-md px-3 py-2 bg-amber-50 text-amber-700 text-xs">
                                  ⚠ Última edición disponible — después de guardar no podrá
                                  modificarse.
                                </div>
                              );
                            return (
                          ...
                          {(reg?.edit_count ?? 0) >= 2 ? (
                            ...
                            <>
                                ...
                                <div className="grid grid-cols-3 gap-1">
                                  {TIPOS_INASISTENCIA.map((ti) => (
                                    <button
                                      ...
                                      className={cn(
                                        "px-2 py-1 text-[10px] rounded-md border text-left transition-colors",
                                        editTipo === ti.code
                              ...
                              <div className="flex gap-2 justify-end">
                                ...
                                <button
                                  onClick={() =>
                                    editarAsistenciaMutation.mutate({
                                      colaborador_id: colabModal.info.colaborador_id,
                                      fecha: iso,
                                      estado: editEstado,
                                      tipo_inasistencia:
                                        editEstado === "Inasistencia" ? editTipo : undefined,
                                      notas: editNotas || undefined,
                                    })
                                  }
                                  disabled={
                                    (editEstado === "Inasistencia" && !editTipo) ||
                                    editarAsistenciaMutation.isPending
                                  }
              ...
              {/* Resumen */}
              {(() => {
                const presentes = dias.filter(
                  (d) => colabModal.dias.get(toISO(d))?.estado === "Presente",
                ).length;
                const inasistencias = dias.filter((d) => {
                  const r = colabModal.dias.get(toISO(d));
                  ...
                  <div className="mt-4 pt-3 border-t border-border flex flex-wrap gap-4">
                    ...
                    {inasistencias > 0 && (
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-xs text-muted-foreground">
                          {inasistencias} inasistencia{inasistencias !== 1 ? "s" : ""}
                        </span>
                      </div>
                    )}
                    {sinRegistro > 0 && (
