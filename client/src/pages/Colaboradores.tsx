...
import { calcularAntiguedad, cn, formatFecha } from "@client/lib/utils";
...
import { ArrowLeft, Camera, Edit2, Search, Trash2, UserMinus } from "lucide-react";
...
export default function Colaboradores() {
  ...
  const { data, isLoading } = useQuery<{ colaboradores: Colaborador[] }>({
    ...
    queryFn: () =>
      fetch("/api/v1/colaboradores?activo=true", { credentials: "include" }).then((r) => r.json()),
  });
            ...
            filas.map((c) => (
              <div key={c.id} className="rounded-xl border border-border bg-card p-4 space-y-3">
                ...
                <div className="text-xs space-y-0.5 text-muted-foreground">
                  ...
                  <p>
                    <span className="font-medium text-foreground">
                      {t("colaboradores:seniorityLabel")}
                    </span>
                    {calcularAntiguedad(c.fecha_ingreso)}
                  </p>
                </div>
              </div>
            ))
          ...
          <div className="bg-card rounded-xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <form
              onSubmit={editForm.handleSubmit((d) =>
                editMutation.mutate({ id: editando.id, data: d }),
              )}
            ...
            >
              ...
              <Field
                ...
                error={editForm.formState.errors.departamento?.message}
              >
                <Combobox
                  options={[...DEPARTAMENTOS_LIST]}
                  value={editForm.watch("departamento") ?? ""}
                  onChange={(v) => editForm.setValue("departamento", v, { shouldValidate: true })}
              ...
              <Field label={t("puesto")}>
                <Combobox
                  ...
                  onChange={(v) => editForm.setValue("puesto", v)}
              ...
              <div className="grid grid-cols-2 gap-3">
                <Field label={t("turno")}>
                  <select {...editForm.register("turno")} className={inputCls}>
                    ...
                    {editando.turno &&
                      !TURNOS.includes(editando.turno as (typeof TURNOS)[number]) && (
          ...
          <div className="bg-card rounded-xl shadow-xl w-full max-w-sm">
            ...
            <div className="p-5 space-y-3">
              <p className="text-sm">
                ...
                <span className="font-medium">{eliminarTarget.fullname}</span>?
              </p>
            ...
            <div className="p-4 border-t border-border flex justify-end gap-2">
              ...
              <button
                onClick={() => eliminarMutation.mutate(eliminarTarget.id)}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-card rounded-xl shadow-xl w-full max-w-sm">
            ...
            <div className="p-5 space-y-4">
              <p className="text-sm">
                <span className="font-medium">{bajaTarget.fullname}</span> —{" "}
                {bajaTarget.departamento}
              </p>
            ...
            <div className="p-4 border-t border-border flex justify-end gap-2">
              ...
              <button
                ...
                disabled={!tipoBaja || bajaMutation.isPending}
                className="px-4 py-2 text-sm rounded-md bg-destructive text-destructive-foreground hover:bg-destructive/90 disabled:opacity-50 transition-colors"
              >
                {bajaMutation.isPending
                  ? t("colaboradores:savingDots")
                  : t("colaboradores:terminate")}
              </button>
            </div>
          </div>
        </div>
