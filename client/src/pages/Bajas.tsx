...
export default function Bajas() {
  ...
  const listaActual = deptActual ? bajas.filter((c) => c.departamento === deptActual) : [];
      ...
      <div className="p-6">
        ...
        <div className="flex items-center gap-3 mb-5">
          <div
            ...
            style={{ backgroundColor: DEPT_COLORS[deptActual] ?? "#888" }}
          />
          <h2 className="text-lg font-semibold">{deptActual}</h2>
          <span className="text-sm text-muted-foreground">
            {"(" + listaActual.length + " " + t("bajas:terminations") + ")"}
          </span>
        </div>
              ...
              <div key={c.id} className="rounded-xl border border-border bg-card p-4 space-y-3">
                ...
                <div className="text-xs space-y-1 text-muted-foreground">
                  <div className="flex gap-2">
                    <span className="font-medium text-foreground">{t("bajas:typeLabel")}</span>
                    {c.tipo_baja ?? "—"}
                  </div>
                  <div className="flex gap-2">
                    <span className="font-medium text-foreground">{t("bajas:dateLabel")}</span>
                    {formatFecha(c.fecha_baja)}
                  </div>
                  <div className="flex gap-2">
                    <span className="font-medium text-foreground">{t("bajas:seniorityLabel")}</span>
                    {calcularAntiguedad(c.fecha_ingreso)}
                  </div>
                  {c.numero_empleado && (
                    <div className="flex gap-2">
                      <span className="font-medium text-foreground">{t("bajas:payrollLabel")}</span>
                      {c.numero_empleado}
                    </div>
                  )}
                  {c.dado_de_baja_por && (
  ...
  return (
    <div className="p-6">
      ...
      <p className="text-sm text-muted-foreground mb-5">
        {t("bajas:subtitle_other", { count: bajas.length })}
      </p>
